<x-app-layout>
    <div class="flex justify-center items-center h-screen">
        <div class="relative w-full max-w-md p-8 bg-gray-950 border-2 border-cyan-500 rounded-none shadow-[0_0_20px_rgba(6,182,212,0.5)] tracking-wider my-12 mx-auto">

            <div class="absolute inset-0 opacity-5 pointer-events-none"
                style="background-size: 20px 20px; background-image: linear-gradient(to right, #06b6d4 1px, transparent 1px), linear-gradient(to bottom, #06b6d4 1px, transparent 1px);"></div>
            ...
            <div class="text-center mb-8 relative">
                ...
                <div class="text-xs text-cyan-400 font-mono mt-2 uppercase tracking-[0.2em]">
                    // Create New Grid Identity v2.1
                </div>
            </div>

            <form method="POST" action="{{ url('/register') }}" class="space-y-6 relative z-10">
                @csrf

                @if ($errors->any())
                    <div class="p-3 bg-red-950/80 border border-pink-600 text-pink-400 font-mono text-xs shadow-[0_0_10px_rgba(219,39,119,0.3)]">
                        <span class="text-pink-500 font-bold">[REJECTION_ERROR]</span> {{ $errors->first() }}
                    </div>
                @endif
                ...
                <div class="flex items-center justify-between font-mono text-xs">
                    <a href="{{ url('/login') }}" class="text-cyan-400 hover:text-cyan-300 isset-link underline underline-offset-4 decoration-pink-500 transition-colors duration-150">
                        Return to Login
                    </a>
                </div>
